...
func start_boon():  # boon effects go here
	boon_manager.active_boons[data_manager.boon_dict[boon_id][1]] = boon_timer
	host.get_node('Head').get_node('Camera3D').get_node('Equipment Manager').reload_mult = 0.5
	host.get_node('Head').get_node('Camera3D').get_node('Equipment Manager').prim_fire_rate_mult = 0.6
	
	
func end_boon():  # undoing boon effects goes here
	print('boon ended | id = ' + boon_id)
	boon_manager.active_boons.erase(data_manager.boon_dict[boon_id][1])
	host.get_node('Head').get_node('Camera3D').get_node('Equipment Manager').reload_mult = 1
	host.get_node('Head').get_node('Camera3D').get_node('Equipment Manager').prim_fire_rate_mult = 1
	queue_free()
